...
- (instancetype)init:(nullable CBCentralManager *)centralManager {
    self = [super init];
    if (self) {
...
// 通过 Delegate 输出日志。
//(NSString *format, ...) NS_FORMAT_FUNCTION(1,2) NS_NO_TAIL_CALL;
- (void)log:(NSString *)format, ... NS_FORMAT_FUNCTION(1,2) {
    va_list args;
    va_start(args, format);
    NSString *content = [[NSString alloc] initWithFormat:format arguments:args];
    if (_blufiDelegate && [_blufiDelegate respondsToSelector:@selector(blufi:log:)]) {
        [_blufiDelegate blufi:self log:content];
    }
}
...
- (void)scanBLE {
    [self log:@"Blufi Scan device: %@", _identifier];
...
- (void)gattWrite:(NSData *)data {
    ...
    if (DBUG) {
        [self log:@"Blufi GattWrite Length: %lu,  %@", (unsigned long)data.length, data];
    }
    if (_writeChar == nil) {
        [self log:@"_writeChar is nil"];
        return;
    }
...
- (BOOL)receiveAck:(Byte)expectAck {
        [self log:@"receiveAck expect: %d", expectAck];
    NSNumber *number = [_deviceAck dequeue];
    if (!number) {
        [self log:@"receiveAck nil"];
        return NO;
    }
    Byte ack = number.intValue;
    [self log:@"receiveAck: %d", ack];
        ...
        if (available > 0 && available <= 2) {
            ...
            if (read != available) {
                // Impossiable come here
                    [self log:@"postContainData: read last bytes error: read=%ld, expect=%ld", (long)read, (long)available];
            }
...
- (NotifyStatus)parseNotification:(NSData *)response notification:(BlufiNotifyData *)notification {
    if (!response) {
        [self log:@"parseNotification nil response"];
        return NotifyNull;
    }
    if (DBUG) {
        [self log:@"Notification: %@", response];
    }
    
    if (response.length < 4) {
        [self log:@"parseNotification invalid length"];
    ...
    [self log: @"parseNotification sequence:%hhu expectSequence:%hhu ", sequence, expectSequence];
    ...
    if (dataLen + dataOffset > response.length) {
        [self log:@"parseNotification invalid data length"];
        return NotifyError;
    }
    ...
    if (frameCtrlData.isChecksum) {
        ...
        if (respChecksum1 != calcChecksum1 || respChecksum2 != calcChecksum2) {
            [self log:@"parseNotification invalid checksum"];
...
- (void)parseWifiState:(NSData *)data {
    ...
    if (data.length < 3) {
        ...
        while (dataIS.hasBytesAvailable) {
            ...
            if (read != 2) {
                [self log:@"parseWifiState contain invalid data1"];
                code = StatusInvalidData;
                break;
            }
            ...
            if (read != len) {
                [self log:@"parseWifiState contain invalid data2"];
                code = StatusInvalidData;
                break;
            }
...
- (void)parseWiFiScanList:(NSData *)data {
    ...
    while (dataIS.hasBytesAvailable) {
        NSInteger read = [dataIS read:temp maxLength:2];
        if (read != 2) {
            [self log:@"parseWiFiScanList contain invalid data1"];
            break;
        }
        Byte length = temp[0];
        if (length < 1) {
            [self log:@"parseWiFiScanList invalid length"];
            break;
        }
        Byte rssi = temp[1];
        read = [dataIS read:temp maxLength:length - 1];
        if (read != length - 1) {
            [self log:@"parseWiFiScanList invalid ssid data"];
            break;
        }
...
- (void)requestDeviceVersion {
    ...
    [_requestQueue addOperationWithBlock:^{
        ...
        if (!posted) {
            [self log:@"Post DeiviceVersion request failed"];
...
- (void)requestDeviceStatus {
    ...
    [_requestQueue addOperationWithBlock:^{
        ...
        BOOL posted = [self post:nil encrypt:encrypted checksum:checksum requireAck:false type:type];
        if (!posted) {
            [self log:@"Post DeviceStatus request failed"];
...
- (void)requestDeviceScan {
    ...
    [_requestQueue addOperationWithBlock:^{
        ...
        BOOL posted = [self post:nil encrypt:encrypted checksum:checksum requireAck:false type:type];
        if (!posted) {
            [self log:@"Post WiFiScan request failed"];
    ...
    [_requestQueue addOperationWithBlock:^{
        ...
        switch (opMode) {
            ...
            case OpModeStaSoftAP:
                ...
                break;
            default:
                [self log:@"configure invalid OpMode: %d", opMode];
...
- (BlufiDH *)postNegotiateSecurity {
    ...
    if (!posted) {
        [self log:@"postNegotiateSecurity: Post length failed"];
        return nil;
    }
    ...
    if (!posted) {
        [self log:@"postNegotiateSecurity: Post data failed"];
        return nil;
    }
...
- (void)negotiateSecurity {
    [_requestQueue addOperationWithBlock:^{
        ...
        @try {
            ...
            [self log:@"negotiateSecurity DH posted"];
            
            NSData *deviceKey = [self.deviceKey dequeue];
            if (!deviceKey) {
                [self log:@"negotiateSecurity Recevie nil deviceKey"];
                code = StatusFailed;
                return;
            }
            ...
            if (DBUG) {
                [self log:@"DH Secret = %@", secretKey];
                [self log:@"AES Key   = %@", self.aesKey];
            }
            
            setSecurity = [self postSetSecurityCtrlEncrypted:NO ctrlChecksum:NO dataEncrypted:YES dataChecksum:YES];
            if (!setSecurity) {
                [self log:@"negotiateSecurity postSetSecurity failed"];
                code = StatusWriteFailed;
            }
        } @catch (NSException *exception) {
            [self log:@"negotiateSecurity exception: %@", exception];
...
- (void)centralManagerDidUpdateState:(nonnull CBCentralManager *)central {
    ...
    if (_blePowerOn) {
        [self log:@"Blufi Client BLE state pwoered on"];
        if (_bleConnectMark) {
            _bleConnectMark = NO;
//            [self scanBLE];
        }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
    // Discover services
    if (error) {
        [self log:@"didDiscoverServices error: %@", error];
        [self clearConnection];
        ...
        if (_service) {
            ...
            [self log:@"didDiscoverServices failed"];
            [self gattDiscoverCallback];
            [self clearConnection];
        }
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    if (error) {
        [self log:@"didDiscoverCharacteristicsForService error: %@", error];
        [self gattDiscoverCallback];
        [self clearConnection];
    } else {
        ...
        for (CBCharacteristic *c in characteristics) {
            if ([c.UUID isEqual:_writeUUID]) {
                [self log:@"didDiscoverCharacteristicsForService get write char"];
                writeChar = c;
            } else if ([c.UUID isEqual:_notifyUUID]) {
                [self log:@"didDiscoverCharacteristicsForService get notify char"];
                notifyChar = c;
            }
        }
        ...
        if (!writeChar || !notifyChar) {
            [self log:@"didDiscoverCharacteristicsForService failed"];
            [self gattDiscoverCallback];
            [self clearConnection];
        } else {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    // Set Notification
    if (error) {
        [self log:@"didUpdateNotificationStateForCharacteristic error: %@", error];
        [self gattDiscoverCallback];
        [self clearConnection];
    } else {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    if (error) {
        [self log:@"didUpdateValueForCharacteristic error: %@", error];
        [self clearConnection];
    } else {
        if (!_notifyData) {
        ...
        switch (status) {
            case NotifyComplete:
                ...
                break;
            case NotifyHasFrag:
                [self log:@"parseNotification wait next"];
                break;
            default:
                [self log:@"parseNotification failed"];
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    [_writeCondition unlock];
    if (error) {
        [self log:@"didWriteValueForCharacteristic error: %@", error];
        [self clearConnection];
    }
